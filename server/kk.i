...
# 1 "/usr/lib/gcc/i686-redhat-linux/4.5.1/include/stdint.h" 1 3 4
...
# 4 "/usr/lib/gcc/i686-redhat-linux/4.5.1/include/stdint.h" 2 3 4
...
int server_init(void)
{
    ...
    if ((sock_server = socket(2, SOCK_STREAM, 0)) < 0) {
        printf("socket error\n");
        exit(1);
    } else {
        printf("socket ok\n");
    ...
    server_addr.sin_addr.s_addr = htons(((in_addr_t) 0x00000000));
...
void daccept(int socket_bind)
{
    ...
    while(1)
    {
        do { int __d0, __d1; __asm__ __volatile__ ("cld; rep; stosl" : "=c" (__d0), "=D" (__d1) : "a" (0), "0" (sizeof (fd_set) / sizeof (__fd_mask)), "1" (&((&fds_set)->__fds_bits)[0]) : "memory"); } while (0);
        ((void) (((&fds_set)->__fds_bits)[((socket_bind) / (8 * (int) sizeof (__fd_mask)))] |= ((__fd_mask) 1 << ((socket_bind) % (8 * (int) sizeof (__fd_mask))))));
        max_fds = socket_bind;
        ret = select(max_fds+1, &fds_set, ((void *)0) , ((void *)0), &timeout);
        if(ret < 0) {
            printf("select error\n");
        } else if(ret == 0) {
            ...
            printf("select ok\n");
            if(((((&fds_set)->__fds_bits)[((socket_bind) / (8 * (int) sizeof (__fd_mask)))] & ((__fd_mask) 1 << ((socket_bind) % (8 * (int) sizeof (__fd_mask))))) != 0)) {
